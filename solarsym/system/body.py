...
class SystemBody:
    ...
    def draw(self, screen, delta: float, zoom: float, center: list[float, float], is_hovered: bool = False):
        """
        Draw the body on the screen
        """
        # Normalize the postition
        screen_pos = [
            (self._position[0] - center[0]) * SystemBody.SYSTEM_SCALE * zoom + screen.get_width() / 2,
            (self._position[1] - center[1]) * SystemBody.SYSTEM_SCALE * zoom + screen.get_height() / 2
        ]
    ...
    def calc_gravity_at_point(self, point: list[float, float]) -> SystemBody.GravityInfo:
        """
        ...
        return SystemBody.GravityInfo(
            force=force,
            direction=[dist_x / dist, dist_y / dist]
        )
    ...
    def calc_collision_bias(self, other: SystemBody) -> CollisionInfo | None:
        ...
        col_normal = [
            rel_pos[0] / rel_pos_mag,
            rel_pos[1] / rel_pos_mag
        ]
        ...
        rel_vel_normalized = [
            rel_vel[0] / rel_vel_mag,
            rel_vel[1] / rel_vel_mag
        ]
        ...
        info = SystemBody.CollisionInfo(
            ...
            bias=bias,
            impulse=impulse,
            reflect_dir=reflection
        )

        return info
    
    def calc_blended_color(self, other: SystemBody, bias: float = 0.5) -> list[float, float, float]:
        """
