...
class SystemBodyPool:
    ...
    def intersects_point(self, point: list[float, float]) -> bool:
        ...
        for body in self.bodies:
            dist_sqr = (point[0] - body.position[0]) ** 2 + (
    ...
    def draw_all(self, screen, delta: float, view_pos: list[float, float], zoom: float):
        ...
        for body in self.bodies:
            body.draw(
                screen, delta, zoom, view_pos, self.intersects_body(body, abs_point)
            )
    ...
    def update_system(self):
        ...
        for affected in self.bodies:
            ...
            for affector in self.bodies:
                ...
                affect_angle = math.atan2(
                    affected.position[1] - affector.position[1],
                    affected.position[0] - affector.position[0],
                )
        ...
        for this_update in pending_updates:
            for affector, col_info in this_update.affectors:
                ...
                reflection_theta = math.atan2(
                    col_info.reflect_dir[1], col_info.reflect_dir[0]
                )
                if (
                    math.fabs(reflection_theta) >= math.pi - 0.05
                    or math.fabs(reflection_theta) < 0.05
                    or col_info.impulse < 10
                ):
                ...
                this_update.due_acceleration = [
                    this_update.due_acceleration[0]
                    + col_info.impulse * col_info.reflect_dir[0],
                    this_update.due_acceleration[1]
                    + col_info.impulse * col_info.reflect_dir[1],
                ]

                this_update.new_color = this_update.affected.calc_blended_color(
                    affector, col_info.bias
                )
        ...
        for body1, body2 in body_merge_info:
            ...
            new_position = [
                (body1.position[0] * body1.mass + body2.position[0] * body2.mass)
                / new_mass,
                (body1.position[1] * body1.mass + body2.position[1] * body2.mass)
                / new_mass,
            ]
            new_velocity = [
                (body1.velocity[0] * body1.mass + body2.velocity[0] * body2.mass)
                / new_mass,
                (body1.velocity[1] * body1.mass + body2.velocity[1] * body2.mass)
                / new_mass,
            ]
            new_body = SystemBody(
                f"{body1.name}_{body2.name}",
                new_mass,
                new_radius,
                new_color,
                new_position,
    ...
    def update_heatmap(self, delta: float, scale: float, view_rect: pygame.Rect):
        ...
        for y in range(
            SystemBodyPool.HEATMAP_CELL_SIZE // 2,
            view_rect.height + SystemBodyPool.HEATMAP_CELL_SIZE // 2,
            SystemBodyPool.HEATMAP_CELL_SIZE,
        ):
            row = []
            for x in range(
                SystemBodyPool.HEATMAP_CELL_SIZE // 2,
                view_rect.width + SystemBodyPool.HEATMAP_CELL_SIZE // 2,
                SystemBodyPool.HEATMAP_CELL_SIZE,
            ):
                point = [
                    (x - view_rect.width // 2) / (SystemBody.SYSTEM_SCALE * scale)
                    + view_rect.x,
                    (y - view_rect.height // 2) / (SystemBody.SYSTEM_SCALE * scale)
                    + view_rect.y,
                ]
    ...
    def create_heatmap_surface(self, view_rect: pygame.Rect) -> pygame.Surface:
        ...
        accel_max = math.log(accel_max)
        accel_min = math.log(accel_min)
        data_range = accel_max - accel_min
